    ...
    <main className="flex h-screen flex-col items-center justify-center">
      <section className='chatbot-section flex flex-col origin:w-[800px] w-full origin:h-[735px] h-full rounded-md p-2 md:p-6'>
        <div className='chatbot-header pb-6'>
          <div className='flex justify-between'>
            <div className='flex items-center gap-2'>
              <h1 className='chatbot-text-primary text-xl md:text-2xl font-medium'>モシモシくん</h1>
            </div>
          ...
          <p className="chatbot-text-secondary-inverse text-sm md:text-base mt-2 md:mt-4">以下のチャットで電話対応履歴に対する検索・質問ができます。</p>
        </div>
        ...
        <form className='flex h-[40px] gap-2' onSubmit={handleSend}>
          <input onChange={handleInputChange} value={input} className='chatbot-input flex-1 text-sm md:text-base outline-none bg-transparent rounded-md p-2' placeholder='検索内容を入力してください...' />
          <button type="submit" className='chatbot-send-button flex rounded-md items-center justify-center px-2.5 origin:px-3'>
            ...
            <span className='hidden origin:block font-semibold text-sm ml-2'>送信</span>
          </button>
        </form>
        <Footer />
      </section>
